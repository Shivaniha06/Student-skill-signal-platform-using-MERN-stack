...
const Dashboard = ({ data, onBack, onViewResume }) => {
  ...
  // 1. Fixed PDF Export with visual feedback
  ...
  // 2. Native Share API (WhatsApp, Telegram, etc.)
  const handleShare = async () => {
    const shareData = {
      title: 'SkillSpectra Analysis',
      text: `Check out ${data.user}'s GitHub impact score of ${data.points}!`,
      url: `${window.location.origin}${window.location.pathname}?user=${data.user}`,
    };
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 md:p-8 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          {/* <button 
            ...
            className="flex items-center gap-2 px-4 py-2 text-xs font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all shadow-md shadow-indigo-100 disabled:opacity-50"
          >
      ...
      <div id="pdf-report" className="space-y-6">
        ...
        {/* Profile Card - Minimalist Sidebar Style */}
        <div className="bg-white border border-slate-200 rounded-2xl p-6 flex flex-col md:flex-row items-center gap-6 shadow-sm">
          <a href={`https://github.com/${data.user}`} target="_blank" rel="noreferrer" className="relative group">
            <img
              src={data.avatar}
              className="w-20 h-20 rounded-full border-2 border-slate-100 group-hover:border-indigo-200 transition-all"
              alt="Avatar"
            />
          ...
          <div className="flex-1 text-center md:text-left">
            <h2 className="text-2xl font-black text-slate-900 flex items-center justify-center md:justify-start gap-2">
              {data.user}
              <a href={`https://github.com/${data.user}`} target="_blank" rel="noreferrer">
                <ExternalLink size={18} className="text-slate-300 hover:text-indigo-600" />
              </a>
            </h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: 'Total Repos', value: data.repo_count },
            { label: 'Publicity', value: '100%' },
        ...
        <div className="flex items-center gap-2 px-2">
          ...
          <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Repository Explorer</h3>
          <div className="h-px bg-slate-200 flex-1"></div>
        </div>
